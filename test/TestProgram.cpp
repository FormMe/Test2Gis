...
namespace test
{
	...
	ResultCode TestProgram::count(const std::string &word, int &cnt) const
	{
		...
		while (!fin.eof())
		{
			...
			возможного переполнения в случае считывания файла целиком
	...
	что нет неообходимости создавать экземляр класса для этой цели
	...
	/*метод осуществляет попытку открыть ранее заданный файл
	...
	fin - поток который осуществляет открытие файла
